...
실제로 title/aria-describedby 속성값에 공백/한글이 포함되어 있을 경우 작동하지 않으므로, 공백을 문자열로 치환하고 한글도 이스케이프 문자로 치환하는 로직을 구현해야 한다. 일단 tooltip 구현 예시를 제시하는 것에 중점을 맞추려고 해당 과정은 생략하였음. 그래놓고 블로그 JS에는 잘만 적용해놓은..
